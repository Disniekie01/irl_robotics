import { ProCallout } from "@/components/callout/irl-pro";
...
export function AITrainingPage() {
  ...
  const handleTrainModel = async () => {
    ...
    try {
      ...
      let response;
      if (selectedModelType === "smolvla_local" || selectedModelType === "gr00t_local") {
        const localBody = {
          model_type: selectedModelType.replace("_local", ""),
          dataset_name: selectedDataset,
          model_name: trainingParams.model_name || null,
          training_params: trainingParams,
          upload_to_hf: !!trainingParams.model_name,
          hf_token: null,
          wandb_api_key: null,
        };
        response = await fetchWithBaseUrl(
          "/training/start-local",
          "POST",
          localBody,
        );
        if (response?.log_file) {
          setCurrentLogFile(response.log_file);
          setShowLogs(true);
        }
      } else if (selectedModelType !== "custom") {
        const trainingBody = {
          model_type: selectedModelType,
          dataset_name: selectedDataset,
    ...
    <div className="container mx-auto py-2 flex flex-col gap-2">
      ...
      <Card className="w-full">
        <CardContent>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="smolvla_local">SmolVLA (local GPU)</SelectItem>
                    <SelectItem value="gr00t_local">gr00t-n1.5 (local GPU)</SelectItem>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <Lightbulb
                          className={`size-5 ${
                            lightbulbOn ? "text-indigo-400 animate-pulse" : ""
                          }`}
                        />
                      </Button>
                    </TooltipTrigger>
          ...
          <div className="flex gap-2 items-center mt-4">
            ...
            >
              ...
              {trainingState === "success" && (
                <>
                  Training started
                  <CheckCircle2 className="size-4 mr-2 text-indigo-400" />
                </>
              )}
            </Button>
          ...
          {(selectedModelType === "custom" || selectedModelType === "smolvla_local" || selectedModelType === "gr00t_local") && (showLogs || currentLogFile) && (
            <LogStream
