# pip install --upgrade irl_robotics
# /// script
...
from irl_robotics.am import Gr00tN1
import httpx
...
# Connect to the irl_robotics server, this is different from the server IP above
...
while True:
    ...
    response = httpx.post(f"{IRL_ROBOTICS_API_URL}/joints/read").json()
    ...
    for i in range(0, action.shape[0]):
        httpx.post(
            f"{IRL_ROBOTICS_API_URL}/joints/write", json={"angles": action[i].tolist()}
        )
