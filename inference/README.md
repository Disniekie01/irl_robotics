...
- a SO-100 controlled by irl_robotics
...
3. Make sure the irl_robotics server is running

```bash
# Install it this way
curl -fsSL https://raw.githubusercontent.com/irl-robotics/irl_robotics/main/install.sh | bash
# Start it this way
irl_robotics run
```

4. Use the `ACT/client.py` script to see how you connect to your model and to your irl_robotics server. Change the serverl URL, port, and camera setup if needed.
...
You can implement the `ActionModel` class with you own logic [here](irl_robotics/am/models.py).

At this point, go to your irl_robotics dashboard > docs and launch the auto/start endpoint that will communicate with the inference server to automatically control the robot.
...
If running everything on the same machine, to avoid conflicts, you may need to disable camera access of the irl_robotics server with the "--no-cameras" flag. 
