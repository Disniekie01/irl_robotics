...
**irl_robotics** – CLI Toolkit for Robot Teleoperation and Action Models
[![PyPI version](https://img.shields.io/pypi/v/irl_robotics?style=flat-square)](https://pypi.org/project/irl_robotics/)
[![GitHub](https://img.shields.io/badge/GitHub-Repo-blue?style=flat-square)](https://github.com/irl-robotics/irl_robotics)
...
- **Skill Graph Editor**: Visual node-based workflow builder with conditions, loops, parallel execution, AI inference, scheduling, and webhook triggers
- **Model Training & Inference**: Kick off training jobs or serve models through HTTP/WebSocket APIs

## irl_robotics installation
...
# Add irl_robotics to your project
uv add irl_robotics
```
...
```bash
pip install irl_robotics
```

## Install from source

For development or if you face issues with some submodule or version, you can install irl_robotics from source.

1. **Clone the irl_robotics repo.** Make sure you have [git lfs](https://git-lfs.com) installed beforehand.
...
4. **Build the app.** (Linux and MacOS) From the root of the repo, run:
   ...
   ```bash
   cd irl_robotics && uv run irl_robotics run --simulation=headless
   ```
...
This adds a [forked version of pybullet](https://github.com/irl-robotics/bullet3) with a modified `examples/ThirdPartyLibs/zlib/zutil.h` to comments the lines with `#define fdopen(fd, mode) NULL` that break pybullet compilation on some verisons of MacOS.

3. Go to the root of this github repo. Enable the python environment created by uv

```bash
source irl_robotics/.venv/bin/activate
```
...
5. Edit the `irl_robotics/pyproject.toml` to uncomment the following line:
...
(Optionnal) Also edit `irl_robotics/simulation/pybullet/pyproject.toml`to uncomment the following line if you want to use the `make prod_gui` command

```bash
[tool.uv.sources]
# Troubleshooting: on MacOS Silicon, you may need to compile pybullet from source.
# If so, follow the guide in the README.md file and uncomment the line below to run make prod_gui
pybullet = { path = "../../../irl_robotics/bullet3", editable = true } # <-- uncomment!
```

6. Run `make` again. You should now see logs similar to this. Note that the pybullet version is now tagged as `irl_robotics version`. The date and time should also match.

```bash
      Built pybullet @ file:///Users/nicolasoulianov/robots/robots/phospho
      Built irl_robotics @ file:///Users/nicolasoulianov/robot
Uninstalled 3 packages in 276ms
Installed 2 packages in 2ms
2025-07-16 23:04:07.468 | INFO     | irl_robotics.main:<module>:4 - Starting irl_robotics...
sys.stdout.encoding = utf-8
    ...
    irl_robotics 0.3.61
    Copyright (c) 2025 IRL Robotics https://phospho.ai

2025-07-16 23:04:08.399 | DEBUG    | irl_robotics.utils:get_tokens:354 - Loaded dev tokens
pybullet build time: Jul 16 2025 23:03:57 (irl_robotics version)
```
...
You can extend **irl_robotics** by plugging in support for any custom robot. Just follow these steps to install irl_robotics from source on any OS:

1. **Install irl_robotics from source** (see instructions just above)

2. **Run irl_robotics with simulation GUI** When running irl_robotics, use the `--simulation=gui` flag to display the pybullet GUI in a new window. This way, you can check if keyboard control and VR control actually work in simulation before trying it on hardware.

3. **Use the URDF Loader to try you robot**. irl_robotics uses [pybullet](https://pybullet.org/wordpress/) ([docs](https://docs.google.com/document/d/10sXEhzFRSnvFcl3XxNGhnD4N2SedqwdAvK3dsihxVUA/edit?tab=t.0)) as a robotics simulation backend.
   ...
   When you're satisfied with your `urdf`, add it to `irl_robotics/resources/urdf`.

4. **Create your robot driver**

   In the directory `irl_robotics/irl_robotics/hardware` add a new file, e.g. `my_robot.py`. Inside, define a class inheriting from `BaseRobot`:

   ```python
   from irl_robotics.hardware.base import BaseRobot
...
5. **Make your robot detectable**
   Open `irl_robotics/irl_robotics/robot.py` and locate the `RobotConnectionManager` class. Make sure your robot can be detected.

   As an alternative, when running irl_robotics, use the `--only-simulation` flag and `config.ONLY_SIMULATION` to force the `RobotConnectionManager` to detect your robot, even if it's not connected to hardware. You'll need to change the `RobotConnectionManager` so that it's actually your robot that gets loaded.
...
- **GitHub Repo**: [https://github.com/irl-robotics/irl_robotics](https://github.com/irl-robotics/irl_robotics)
- **Examples**: Browse [the examples](https://github.com/irl-robotics/irl_robotics/tree/main/examples)
...
Made with 💚 by the IRL Robotics community.
