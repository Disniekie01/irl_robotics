...
def fetch_latest_brew_version(fail_silently: bool = False) -> str:
    ...
    """
    url = "https://raw.githubusercontent.com/irl-robotics/homebrew-irl_robotics/refs/heads/main/Formula/irl_robotics.rb"
    try:
...
def get_home_app_path() -> Path:
    ...
    overridden per machine via IRL_ROBOTICS_HOME.

    Default: Path.home() / "irl_robotics"
    Override: set env IRL_ROBOTICS_HOME to e.g. /home/ubuntu/irl_robotics
    ...
    """
    home_path = Path(
        os.environ.get("IRL_ROBOTICS_HOME", str(Path.home() / "irl_robotics"))
    )
