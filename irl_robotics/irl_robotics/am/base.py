...
from irl_robotics.models import InfoModel, ModelConfigurationResponse
from irl_robotics.utils import get_hf_token
...
class BaseTrainerConfig(BaseModel):
    ...
    model_name: Optional[str] = Field(
        default=None,
        description="Name of the trained model to upload to Hugging Face, should be in the format irl-robotics/<model_name> or <model_name>",
    )
...
class TrainingRequest(BaseTrainerConfig):
    ...
    def prepare_model_and_params(cls, data: dict) -> dict:
        ...
        if user_provided_name:
            ...
            # If the namespace is different from irl-robotics and not in private mode, raise error
            if namespace != "irl-robotics" and not data.get("private_mode"):
                raise ValueError(
                    "You can only use a custom model name in private mode or with the irl-robotics namespace."
                )
...
# {model_type} model - 🧪 irl_robotics training pipeline
...
"""
        ...
        readme += """
## This model was trained using **[🧪IRL Robotics](https://irlrobotics.ai)**
...
📖 **Get Started**: [docs.irlrobotics.ai](https://docs.irlrobotics.ai?utm_source=huggingface_readme)

🤖 **Get your robot**: [robots.irlrobotics.ai](https://robots.irlrobotics.ai?utm_source=huggingface_readme)
"""
