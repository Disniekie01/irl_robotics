...
from irl_robotics.chat.agent import RoboticAgent
from irl_robotics.chat.utils import KEYBOARD_CONTROl_TEXT, ascii_test_tube
...
class AgentScreen(Screen):
    """
    The main screen for the irl_robotics chat interface.
    ...
    def _write_welcome_message(self) -> None:
        self._write_to_log(
            f"""🧪 Welcome to irl_robotics chat!

{ascii_test_tube()}

[grey46]Access the irl_robotics dashboard here: http://{get_local_ip()}:{config.PORT}
...
class AgentApp(App):
    """
    The main application class for the irl_robotics chat interface.
    This app manages the agent lifecycle, user input, and UI updates.
    """

    TITLE = "irl_robotics chat"
    SUB_TITLE = "Ready"
    ...
    def _handle_prompt(self, prompt: str, screen: AgentScreen) -> None:
        ...
        if prompt.strip().lower() == "/init":
            ...
            asyncio.create_task(self.current_agent.irl_robotics_client.move_init())
            return None
    ...
    async def _run_agent(self, agent: RoboticAgent) -> None:
        ...
        try:
            ...
            # Call stop recording
            await agent.irl_robotics_client.stop_recording()
            self.post_message(
