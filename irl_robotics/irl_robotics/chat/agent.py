...
from irl_robotics.models import ChatRequest, ChatResponse
from irl_robotics.utils import get_local_ip


class IRLRoboticsClient:
    def __init__(
        self,
...
class RoboticAgent:
    def __init__(
        ...
        self.irl_robotics_client = IRLRoboticsClient(write_to_log=write_to_log)
    ...
    async def _get_ai_response(
        ...
        self._current_ai_task = asyncio.create_task(
            self.irl_robotics_client.chat(chat_request=chat_request)
        )
    ...
    async def execute_chat_response(
        ...
        if chat_response.endpoint == "move_relative":
            if not chat_response.endpoint_params:
                ...
                return None
            await self.irl_robotics_client.move_relative(**chat_response.endpoint_params)
        else:
            logger.warning(
    ...
    async def execute_command(self, command: str) -> Optional[Dict[str, float]]:
        ...
        if next_robot_move is None:
            ...
            return None
        # Call the irl_robotics client to move the robot
        await self.irl_robotics_client.move_relative(**next_robot_move)
    ...
    async def run(self) -> AsyncGenerator[Tuple[str, Dict[str, Any]], None]:
        ...
        self.robot_status = await self.irl_robotics_client.status()
        ...
        await self.irl_robotics_client.start_recording(
        ...
        while step_count < max_steps:
            ...
            if self.control_mode == "ai":
                ...
                # Run the agent
                images = await self.irl_robotics_client.get_camera_image(
                    resize=self.resize
                )
                if not images:
                    yield (
                ...
                if not chat_logged:
                    ...
                    await self.irl_robotics_client.log_chat(chat_request=chat_request)
                    chat_logged = True
        ...
        await self.irl_robotics_client.stop_recording()
