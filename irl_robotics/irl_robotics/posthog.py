...
from irl_robotics.utils import get_home_app_path, get_tokens
...
posthog_details = {
    "irl_robotics_version": __version__,
    "env": tokens.ENV,
...
def get_or_create_unique_id(token_path: Path) -> str:
    ...
    if os.path.exists(token_path):
        ...
        if content and content.startswith("irl_robotics_"):
            return content

    # Generate a new unique ID if no token exists
    new_user_id = "irl_robotics_" + str(uuid.uuid4())
...
def posthog_pageview(page: str) -> None:
    posthog.capture(
        ...
        properties={
            **posthog_details,
            "$current_url": "http://irlrobotics.local" + page,
        },
    )
