...
from irl_robotics.configs import config as cfg
from irl_robotics.hardware import get_sim
from irl_robotics.models import BaseRobot, BaseRobotConfig, BaseRobotInfo, Temperature
...
class BaseManipulator(BaseRobot):
    ...
    def init_config(self) -> None:
        ...
        if config is not None:
            self.config = config
            logger.success("Loaded config from home directory irl_robotics.")
            return
    ...
    def _rad_to_open_command(self, radians: float) -> float:
        ...
        close_position = self.config.servos_offsets[-1]
        open_command = (
            self._radians_to_motor_units(radians=radians, servo_id=self.gripper_servo_id)
            - close_position
