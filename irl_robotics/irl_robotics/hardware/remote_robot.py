...
from irl_robotics.models import BaseRobotConfig, RobotConfigStatus


class _WebSocketTransport:
    """
    Persistent WebSocket connection for low-latency joint streaming.
    Replaces per-call HTTP requests for read_joints_position / set_motors_positions.
    """
    ...
    def __init__(self, url: str) -> None:
        ...
        self._last_positions: np.ndarray = np.zeros(6)

    def connect(self) -> None:
        import websockets.sync.client as ws_sync

        self._ws = ws_sync.connect(self._url)
        logger.info(f"WebSocket transport connected to {self._url}")
    ...
    def _reconnect(self) -> bool:
        try:
            ...
            self._ws = ws_sync.connect(self._url)
            logger.info(f"WebSocket transport reconnected to {self._url}")
            return True
    ...
    def read_joints(self) -> np.ndarray:
        ...
        with self._lock:
            ...
            try:
                msg = msgpack.packb({"cmd": "read"})
                self._ws.send(msg)
                reply = msgpack.unpackb(self._ws.recv(), raw=False)
                self._last_positions = np.array(reply["angles"])
                return self._last_positions
            except Exception as e:
    ...
    def write_joints(self, angles: np.ndarray, unit: str = "rad") -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(f"WebSocket write error: {e}")
                self._ws = None


class RemoteRobot(BaseRobot):
    """
    Class to connect to another irl_robotics server.
    Supports two transport modes:
      - "http" (default): one HTTP request per call, compatible with all features
      - "ws": persistent WebSocket for low-latency joint streaming (leader-follower)
    """
    ...
    def __init__(
        ...
        port: int,
        robot_id: int,
        mode: Literal["http", "ws"] = "http",
        **kwargs: dict[str, Any],
    ) -> None:
        """
        Initialize connection to irl_robotics.

        Args:
            ip: IP address of the robot
            port: Port of the remote server
            robot_id: ID of the robot on the remote server
            mode: Transport mode - "http" for REST calls, "ws" for WebSocket streaming
    ...
    async def connect(self) -> None:
        """
        Initialize communication with the irl_robotics server by calling /status.
        If mode is "ws", also opens the WebSocket transport for joint streaming.
        ...
        try:
            ...
            logger.info(f"Connected to remote irl_robotics at {self.ip}:{self.port} (mode={self.mode})")
        except httpx.RequestError as e:
            logger.warning(f"Failed to connect to remote irl_robotics: {e}")
            raise Exception(f"Connection failed: {e}")

        if self.mode == "ws":
            try:
                ws_url = f"ws://{self.ip}:{self.port}/ws/joints?robot_id={self.robot_id}"
                self._ws_transport = _WebSocketTransport(ws_url)
                self._ws_transport.connect()
                logger.info(f"WebSocket transport established to {ws_url}")
            except Exception as e:
                logger.warning(f"WebSocket transport failed, falling back to HTTP: {e}")
    ...
    def disconnect(self) -> None:
        ...
        try:
            ...
            logger.info("Disconnected from remote irl_robotics")
        except Exception as e:
            logger.warning(f"Failed to disconnect from remote irl_robotics: {e}")
    ...
    def read_joints_position(
        ...
        Uses WebSocket transport if available (unit must be 'rad'), otherwise HTTP.
        """
        if not self.is_connected:
            logger.warning("Robot is not connected")
            return np.zeros(6)
        ...
        if (
            self._ws_transport is not None
