...
from irl_robotics.robot import (
    RemoteRobot,
    RobotConnectionManager,
    SO100Hardware,
    get_rcm,
)
from irl_robotics.supabase import get_client, user_is_logged_in
from irl_robotics.teleoperation import (
    TeleopManager,
    UDPServer,
    get_teleop_manager,
    get_udp_server,
)
from irl_robotics.utils import background_task_log_exceptions, get_tokens
...
async def move_teleop_ws(
    ...
    # Accept first: rejecting the handshake with HTTPException makes many clients
    # report an abnormal WebSocket close (e.g. 1006) with no clear reason.
                ...
                json.dumps(
                    {
                        ...
                        "detail": "Connect hardware or start with simulation so a robot appears in the dashboard.",
                    }
                )
            )
        except Exception:
            pass
        await websocket.close(code=1008, reason="No robot connected")
        return
...
_WS_JOINTS_MIN_INTERVAL = 1.0 / 200  # Cap at 200Hz


@router.websocket("/ws/joints")
async def ws_joints_stream(
    websocket: WebSocket,
    robot_id: int = 0,
    rcm: RobotConnectionManager = Depends(get_rcm),
) -> None:
    """
    Bidirectional WebSocket for low-latency joint streaming.
    Used by RemoteRobot in 'ws' mode for network leader-follower control.

    Protocol (msgpack):
      Client sends: {"cmd": "write", "angles": [...], "unit": "rad"}
      Server sends: {"angles": [...], "unit": "rad"} on each client message
      Client sends: {"cmd": "read"} to request current joint positions
    """
    import time
    ...
    await websocket.accept()
    logger.info(f"WebSocket joints stream opened for robot {robot_id}")
    last_msg_time = 0.0
    ...
    try:
        while True:
            ...
            now = time.monotonic()
            if now - last_msg_time < _WS_JOINTS_MIN_INTERVAL:
                await asyncio.sleep(_WS_JOINTS_MIN_INTERVAL - (now - last_msg_time))
            ...
            cmd = msg.get("cmd", "read")

            try:
                if cmd == "write":
                    angles = msg.get("angles", [])
                    unit = msg.get("unit", "rad")
                    if hasattr(robot, "set_motors_positions"):
                        robot.set_motors_positions(
                            q_target_rad=np.array(angles), enable_gripper=False
                        )
                    elif hasattr(robot, "write_joint_positions"):
                        robot.write_joint_positions(angles=angles, unit=unit)

                pos = robot.read_joints_position(unit="rad", source="robot")
                reply = {"angles": pos.tolist(), "unit": "rad"}
            except Exception as e:
                reply = {"angles": [], "unit": "rad", "error": str(e)}

            await websocket.send_bytes(msgpack.packb(reply))

    except WebSocketDisconnect:
        logger.info(f"WebSocket joints stream closed for robot {robot_id}")
    except Exception as e:
        logger.error(f"WebSocket joints stream error: {e}")
        try:
...
async def start_leader_follower(
    ...
    for i, robot_pair in enumerate(request.robot_pairs):
        ...
        if request.enable_gravity_compensation or request.enable_haptic_feedback:
            ...
            valid_robot_types = (BaseManipulator, RemoteRobot)
    ...
    background_tasks.add_task(
        ...
        enable_haptic_feedback=request.enable_haptic_feedback,
        haptic_feedback_strength=request.haptic_feedback_strength,
    )
...
async def add_robot_connection(
    ...
    try:
        ...
        if "Access is denied" in str(e):
            ...
            raise HTTPException(
                status_code=403,
                detail=f"Permission error: {e}. If you're on Windows, try running with WSL (Windows Subsystem for Linux) or irl_robotics has the authorization to use the serial port.",
            )
            ...
            raise HTTPException(
                status_code=403,
                detail=f"Permission denied: {e}. If you're on Linux, try running irl_robotics as sudo (`sudo irl_robotics`) and ensure that your user has access to the serial port.",
            )
        else:
            logger.error(
...
async def get_robot_config(
    ...
    if isinstance(robot, BaseManipulator) or isinstance(robot, RemoteRobot):
        config = robot.config
...
_HAPTIC_PROFILES_FILE = "haptic_profiles.json"
_haptic_profiles_lock = asyncio.Lock()


class HapticProfileSettings(BaseModel):
    haptic_feedback_strength: Optional[int] = None
    gravity_compensation_values: Optional[Dict[str, int]] = None


class SaveHapticProfileRequest(BaseModel):
    name: str = Field(..., min_length=1, max_length=64)
    settings: HapticProfileSettings


def _get_haptic_profiles_path() -> Path:
    from irl_robotics.utils import get_home_app_path

    return get_home_app_path() / _HAPTIC_PROFILES_FILE


@router.get("/control/haptic-profiles")
async def get_haptic_profiles() -> dict:
    path = _get_haptic_profiles_path()
    if not path.exists():
        return {"profiles": {}}
...
@router.post("/control/haptic-profiles")
async def save_haptic_profile(body: SaveHapticProfileRequest) -> StatusResponse:
    async with _haptic_profiles_lock:
        path = _get_haptic_profiles_path()
        existing: dict = {"profiles": {}}
        if path.exists():
            with open(path, "r") as f:
                existing = json.load(f)
        existing["profiles"][body.name] = body.settings.model_dump(exclude_none=True)
        with open(path, "w") as f:
            json.dump(existing, f, indent=2)
    return StatusResponse(message=f"Profile '{body.name}' saved")


@router.delete("/control/haptic-profiles")
async def delete_haptic_profile(name: str) -> StatusResponse:
    async with _haptic_profiles_lock:
        path = _get_haptic_profiles_path()
        if not path.exists():
            return StatusResponse(status="error", message="No profiles file")
        with open(path, "r") as f:
            existing = json.load(f)
        existing.get("profiles", {}).pop(name, None)
        with open(path, "w") as f:
            json.dump(existing, f, indent=2)
    return StatusResponse(message=f"Profile '{name}' deleted")
