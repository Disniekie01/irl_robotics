...
from irl_robotics.utils import fetch_latest_brew_version, is_running_on_pi
    ...
    if platform.system() == "Linux":
        ...
        try:
            ...
            version_process = await asyncio.create_subprocess_shell(
                "apt-cache policy irl_robotics | grep 'Candidate:' | awk '{print $2}'",
        ...
        # Check the brew package version in the repo homebrew-irl_robotics
...
async def upgrade_to_latest_version() -> dict:
    ...
    try:
        ...
        subprocess.run(["sudo", "apt", "upgrade", "-y", "irl_robotics"], check=True)
        return {
