...
- You have recorded a dataset of the SO-100 robot arm in LeRobot dataset format (using irl_robotics or LeRobot).
...
```bash
git clone https://github.com/irl-robotics/irl_robotics.git
cd irl_robotics
```

We are going to use the irl_robotics fork of Isaac-GR00T repo that supports multiple SO-100 as embodiments.

Install the dependencies using conda:

```bash
git clone https://github.com/irl-robotics/Isaac-GR00T.git
...
In the irl_robotics folder, still in the `gr00t` conda environment, run:
...
from irl_robotics.am import Gr00tTrainerConfig, Gr00tTrainer
...
To control your local robot, we will use `irl_robotics`.
If you haven't already, install it with:

```bash
curl -fsSL https://raw.githubusercontent.com/irl-robotics/irl_robotics/main/install.sh | sudo bash
```
...
More information on how to use irl_robotics [here](https://docs.phospho.ai/).
...
Adapt the script `scripts/quickstart_ai_gr00t.py` to your specific setup (robot, cameras, irl_robotics server and inference server).
