...
```bash
cd ./irl_robotics
uv run irl_robotics run --simulation=gui
```
...
Then, you can run irl_robotics and it will display a window. There will likely be visual glitches. 
...
4. In a new terminal, you can now run the main `irl_robotics` server, which handles the controller logic.
