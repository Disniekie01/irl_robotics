...
### Backend
1. **Core Action Model Layer** (`irl_robotics/am/`): Client-side model interfaces and control logic, model-specific validators, and spawn configurations
2. **Modal Infrastructure Layer** (`modal/lerobot_modal/`): Server-side inference and training infrastructure
3. **IRL Robotics Dashboard**: AI Control and AI Training endpoints

### Frontend
1. **IRL Robotics Dashboard** (`dashboard/src`): AI Control and AI Training endpoints
...
Create a new file `irl_robotics/am/your_policy.py` and implement the following:
...
1. **Add a new `TrainingParams` class in `irl_robotics/am/base.py`**:
...
```python
from irl_robotics.am.yourpolicy import YourPolicySpawnConfig
from .helper import validate_inputs, prepare_base_batch
from .app import base_image, irl_robotics_dir, serve_policy, train_policy
```
...
your_policy_image = (
    ...
    .pip_install_from_pyproject(pyproject_toml=str(irl_robotics_dir / "pyproject.toml"))
    .add_local_python_source("irl_robotics")
)
...
```python
from irl_robotics.am.yourpolicy import YourPolicySpawnConfig
from irl_robotics.am.base import TrainingParamsYourPolicy
```
...
#### 5. Make changes to enable AI control and training via irl_robotics dashboard

Files to update:
- `irl_robotics/irl_robotics/endpoints/pages.py`
- `irl_robotics/irl_robotics/models/__init__.py`
- `irl_robotics/irl_robotics/ai_control.py`
