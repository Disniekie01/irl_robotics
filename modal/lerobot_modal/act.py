...
from irl_robotics.am.base import TrainingParamsAct, TrainingParamsActWithBbox
from irl_robotics.models import InfoModel
from irl_robotics.models.lerobot_dataset import FeatureDetails
from irl_robotics.models.lerobot_dataset import LeRobotDataset
from irl_robotics.am.act import ACTSpawnConfig
...
def compute_bboxes(
    ...
    """
    # Load the dataset with irl_robotics to fix episodes.jsonl issues (usually: missing episodes)
...
def upload_bbox_dataset_to_hf(
    ...
    """
    # Use user's namespace for private training, irl-robotics for public
    if private_mode and hf_token:
        ...
        try:
            ...
            if username:
                ...
                logger.warning(
                    "Could not get username from HF token, using irl-robotics namespace"
                )
                dataset_name = "irl-robotics/" + dataset_path.name
        except Exception as e:
            logger.warning(
                f"Error getting username from HF token: {e}, using irl-robotics namespace"
            )
            dataset_name = "irl-robotics/" + dataset_path.name
    else:
        dataset_name = "irl-robotics/" + dataset_path.name
