...
import modal
from irl_robotics.am.base import TrainingParamsGr00T
from irl_robotics.am.gr00t import Gr00tSpawnConfig
...
irl_robotics_dir = (
    Path(__file__).parent.parent.parent.parent.parent / "irl_robotics" / "irl_robotics"
)
...
gr00t_image = (
    ...
    .pip_install_from_pyproject(
        pyproject_toml=str(irl_robotics_dir / "pyproject.toml"),
    )
    .run_commands(
        "git clone https://github.com/irl-robotics/Isaac-GR00T.git /workspace/gr00t && cd /workspace/gr00t && git checkout 2beed498ae6c76f84a5ac0c342dbffa8dbab1e74",
    )
...
def serve(
    ...
    with modal.forward(server_port, unencrypted=True) as tunnel:
        ...
        from irl_robotics.am.gr00t import RobotInferenceServer

        # Handle the code logic here so we don't have to juggle betwwen Gr00t files and irl_robotics files
...
def train(  # All these args should be verified in irl_robotics
    training_id: int,
    dataset_name: str,
